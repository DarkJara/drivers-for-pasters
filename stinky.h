...
private:
	PVOID SharedBuffer;
	HANDLE hDriver;
	ULONG64 MAGGICCODE = 0x32004760413c2e6dul;
...
template <typename T>
T readvm(const uintptr_t address)
{
	T buffer{ };
...
template <typename T>
T writevm(const uintptr_t address, T buffer)
{
	driver->WriteProcessMemory((PVOID)&buffer, (PVOID)address, sizeof(T));
	return buffer;
}
std::string readvm(uintptr_t Address, void* Buffer, SIZE_T Size)
{
...
uint64_t ReadChain(uint64_t base, const std::vector<uint64_t>& offsets) {
	uint64_t result = readvm<uint64_t>(base + offsets.at(0));
	for (int i = 1; i < offsets.size(); i++) {
		result = readvm<uint64_t>(result + offsets.at(i));
	}
	return result;
}